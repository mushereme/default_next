  ...
  return rows.map((item, index) => {
    return (
      <div className="relative relative h-screen" key={index}>
        <div className="relative mr-auto sm:px-6 lg:w-5/12 z-10 h-screen bg-white flex items-center">
              ...
              <div className="sm:text-center lg:text-left">
                {!isEmpty(item?.title?.value) && (
                  <div className="border-l-4 border-gray-900 pl-3">
                    <h4 className="text-4xl tracking-tight font-extralight text-grey-900">
                      {item?.title?.value}
                    </h4>
                  </div>
                )}
                <div className="lg:block md:block hidden">
                  <GoogleDiv2
                    item={item.desc}
                    defaultClassName="text-grey-500 sm:mt-5"
                    isMarkdown={true}
                  />
                </div>
              </div>
        ...
        <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-6/12">
          <div
            style={{
              backgroundColor: "#fff",
